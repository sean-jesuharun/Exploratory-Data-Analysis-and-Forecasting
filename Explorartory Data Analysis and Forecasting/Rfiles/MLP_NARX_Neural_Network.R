...
DataFrame_days <- read.csv("DataFrame_NARX(4).csv")
...
set.seed(4444)
nn <- neuralnet(NextDay_11 ~., data = Data_train_scale, hidden = c(14,8), 
...
#validation
results <- data.frame(actual = Data_test_scale$NextDay_11, prediction = nn.results$net.result)
results

actual = Data_test_NoScale$NextDay_11
head(actual)

predicted = (results$prediction * (max(Data_test_NoScale$NextDay_11) - min(Data_test_NoScale$NextDay_11))) + min(Data_test_NoScale$NextDay_11)
head(predicted)
head(Data_test_NoScale$NextDay_11)

class(actual)
class(predicted)

#validation
results <- data.frame(desired_Output = actual, predicted_Output = predicted)
results
